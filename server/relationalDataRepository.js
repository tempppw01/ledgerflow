...
async function setSetting(database, key, value) {
  ...
  if (database.dialect === 'sqlite') {
    await database.run(
      ...
      [DEFAULT_USER_ID, key, payload, timestamp]
    );
    return;
  }
  await database.run(
...
async function clearUserData(database) {
  // Child rows must be cleared before their referenced business records.
  const statements = [
    'DELETE FROM ai_workflow_context_refs',
    'DELETE FROM ai_workflow_messages',
    ...
    'DELETE FROM ledger_transaction_attachments',
    'DELETE FROM ledger_transaction_tags',
  ...
  for (const sql of statements) {
    await database.run(sql, sql.includes('?') ? [DEFAULT_USER_ID] : []);
  }
}

export async function replaceRelationalData(provider, payload, env = process.env) {
  ...
  await withDatabase(provider, async (database) => {
    ...
    try {
      ...
      await database.run(
        ...
        [DEFAULT_USER_ID, 'Default user', database.date(timestamp), database.date(timestamp)]
      );
      ...
      for (const item of accounts) {
        ...
        await database.run(
          `INSERT INTO ledger_accounts (id, user_id, name, account_type, initial_balance, balance, sort_order, trashed_at, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [id(item.id), DEFAULT_USER_ID, text(item.name, 'Untitled account'), nullable(item.type), number(item.initialBalance), number(item.balance), Math.floor(number(item.sortOrder)), nullable(item.trashedAt), database.date(timestamp), database.date(timestamp)]
        );
      }
      for (const item of categories) {
        ...
        await database.run(
          `INSERT INTO ledger_categories (id, user_id, name, category_kind, color, icon, sort_order, trashed_at, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [id(item.id), DEFAULT_USER_ID, text(item.name, 'Untitled category'), item.kind === 'income' ? 'income' : 'expense', nullable(item.color), nullable(item.icon), Math.floor(number(item.sortOrder)), nullable(item.trashedAt), database.date(timestamp), database.date(timestamp)]
        );
      }
      for (const item of transactions) {
        ...
        await database.run(
          `INSERT INTO ledger_transactions (id, user_id, account_id, category_id, transaction_type, amount, occurred_at, note, source, order_no, merchant_order_no, transaction_status, adjustment_kind, refund_of_transaction_id, trashed_at, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [id(item.id), DEFAULT_USER_ID, id(item.accountId), id(item.categoryId), text(item.type, 'expense'), number(item.amount), database.date(item.date), text(item.note), nullable(item.source), nullable(item.orderNo), nullable(item.merchantOrderNo), nullable(item.status), text(item.adjustmentKind, 'normal'), null, nullable(item.trashedAt), database.date(item.date), database.date(item.updatedAt || item.date)]
        );
      ...
      for (const item of transactions) {
        ...
        await database.run(
          ...
          [id(item.refundOfTransactionId), id(item.id), DEFAULT_USER_ID]
        );
      }
      for (const item of balanceChanges) {
        ...
        await database.run(
          `INSERT INTO ledger_balance_changes (id, user_id, account_id, transaction_id, related_transaction_id, change_type, amount, before_balance, after_balance, note, remark, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [id(item.id), DEFAULT_USER_ID, id(item.accountId), validTransactionIds.has(id(item.transactionId)) ? id(item.transactionId) : null, validTransactionIds.has(id(item.relatedTransactionId)) ? id(item.relatedTransactionId) : null, text(item.type, 'manual-adjustment'), number(item.amount), number(item.beforeBalance), number(item.afterBalance), nullable(item.note), nullable(item.remark), database.date(item.createdAt)]
        );
      }
      for (const item of subscriptions) {
        ...
        await database.run(
          `INSERT INTO ledger_subscriptions (id, user_id, account_id, name, subscription_kind, amount, currency, billing_cycle, custom_cycle_days, provider, note, renewal_date, expire_date, auto_renew, subscription_status, last_generated_at, last_generated_transaction_id, trashed_at, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [id(item.id), DEFAULT_USER_ID, validAccountIds.has(id(item.accountId)) ? id(item.accountId) : null, text(item.name, 'Untitled subscription'), text(item.kind, 'other'), number(item.amount), text(item.currency, 'CNY').slice(0, 16), text(item.billingCycle, 'monthly'), Math.floor(number(item.customCycleDays)) || null, nullable(item.provider), nullable(item.note), nullable(item.renewalDate), nullable(item.expireDate), bool(item.autoRenew), text(item.status, 'active'), nullable(item.lastGeneratedAt), validTransactionIds.has(id(item.lastGeneratedTransactionId)) ? id(item.lastGeneratedTransactionId) : null, nullable(item.trashedAt), database.date(item.createdAt), database.date(item.updatedAt)]
        );
      }
      ...
      for (const item of goals) {
        ...
        await database.run(
          `INSERT INTO investment_goals (id, user_id, name, goal_kind, target_amount, current_amount, monthly_contribution, target_date, priority, note, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [id(item.id), DEFAULT_USER_ID, text(item.name, 'Untitled goal'), text(item.kind, 'other'), number(item.targetAmount), number(item.currentAmount), number(item.monthlyContribution) || null, nullable(item.targetDate), text(item.priority, 'medium'), nullable(item.note), database.date(item.createdAt), database.date(item.updatedAt)]
        );
      }
      for (const item of watchlist) {
        ...
        await database.run(
          `INSERT INTO investment_watchlists (id, user_id, instrument_id, name, code, platform, note, holding_shares, investment_advice, last_verdict, last_summary, last_risk_level, last_analysis_at, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [id(item.id), DEFAULT_USER_ID, null, text(item.name, 'Untitled watch item'), nullable(item.code), nullable(item.platform), nullable(item.note), number(item.holdingShares) || null, nullable(item.investmentAdvice), nullable(item.lastVerdict), nullable(item.lastSummary), nullable(item.lastRiskLevel), nullable(item.lastAnalysisAt), database.date(item.createdAt), database.date(item.updatedAt)]
        );
        for (const tag of strings(item.tags)) {
          const value = tag.slice(0, 64);
      ...
      await setSetting(database, SETTING_KEYS.globalMemories, globalMemories);
      await setSetting(database, SETTING_KEYS.rssSubscriptions, list(preferences.rssSubscriptions));
      await setSetting(database, SETTING_KEYS.investmentAiMessages, list(preferences.investmentAiMessages));
      await setSetting(database, SETTING_KEYS.investmentWatchlistExtras, Object.fromEntries(watchlist.map((item) => [id(item.id), watchlistExtra(item)])));
      await setSetting(database, SETTING_KEYS.monthlyIncome, number(preferences.monthlyIncome));
      await setSetting(database, SETTING_KEYS.categoryLearningRules, list(finance.categoryLearningRules));
      await setSetting(database, SETTING_KEYS.categoryLearningEvents, list(finance.categoryLearningEvents));
  ...
  return getRelationalDataStatus(provider, env);
}

export async function getRelationalDataStatus(provider, env = process.env) {
  return withDatabase(provider, async (database) => {
    ...
    for (const table of tables) {
      const rows = await database.query(`SELECT COUNT(*) AS count FROM ${table} WHERE user_id = ?`, [DEFAULT_USER_ID]);
      counts[table.replace(/^ledger_/, '')] = Number(rows[0]?.count || 0);
    }
    const rows = await database.query('SELECT MAX(updated_at) AS updated_at FROM ledger_user_settings WHERE user_id = ?', [DEFAULT_USER_ID]);
    return { ok: true, hasData: Object.values(counts).some((count) => count > 0), counts, updatedAt: fromDbDate(rows[0]?.updated_at) || null };
  }, env);
}

export async function getRelationalBootstrap(provider, env = process.env) {
  return withDatabase(provider, async (database) => {
    const [accounts, categories, transactions, tags, attachments, balanceChanges, subscriptions, debts, repayments, positions, histories, goals, watchlists, watchTags, settings] = await Promise.all([
      database.query('SELECT * FROM ledger_accounts WHERE user_id = ? ORDER BY sort_order, created_at', [DEFAULT_USER_ID]),
      database.query('SELECT * FROM ledger_categories WHERE user_id = ? ORDER BY sort_order, created_at', [DEFAULT_USER_ID]),
      database.query('SELECT * FROM ledger_transactions WHERE user_id = ? ORDER BY occurred_at, created_at', [DEFAULT_USER_ID]),
      database.query('SELECT t.* FROM ledger_transaction_tags t JOIN ledger_transactions x ON x.id = t.transaction_id WHERE x.user_id = ?', [DEFAULT_USER_ID]),
      database.query('SELECT a.* FROM ledger_transaction_attachments a JOIN ledger_transactions x ON x.id = a.transaction_id WHERE x.user_id = ?', [DEFAULT_USER_ID]),
      database.query('SELECT * FROM ledger_balance_changes WHERE user_id = ? ORDER BY created_at', [DEFAULT_USER_ID]),
      database.query('SELECT * FROM ledger_subscriptions WHERE user_id = ? ORDER BY created_at', [DEFAULT_USER_ID]),
      database.query('SELECT * FROM ledger_debts WHERE user_id = ? ORDER BY created_at', [DEFAULT_USER_ID]),
      database.query('SELECT r.* FROM ledger_debt_repayments r JOIN ledger_debts d ON d.id = r.debt_id WHERE d.user_id = ?', [DEFAULT_USER_ID]),
      database.query('SELECT * FROM investment_positions WHERE user_id = ? ORDER BY created_at DESC', [DEFAULT_USER_ID]),
      database.query('SELECT h.* FROM investment_position_history h JOIN investment_positions p ON p.id = h.position_id WHERE p.user_id = ? ORDER BY h.created_at DESC', [DEFAULT_USER_ID]),
      database.query('SELECT * FROM investment_goals WHERE user_id = ? ORDER BY created_at DESC', [DEFAULT_USER_ID]),
      database.query('SELECT * FROM investment_watchlists WHERE user_id = ? ORDER BY updated_at DESC', [DEFAULT_USER_ID]),
      database.query('SELECT t.* FROM investment_watch_tags t JOIN investment_watchlists w ON w.id = t.watchlist_id WHERE w.user_id = ?', [DEFAULT_USER_ID]),
    ...
    return {
      ok: true,
      ...(await getRelationalDataStatus(provider, env)),
      data: {
